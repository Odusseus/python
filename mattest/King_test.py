...
def test_King_clone_Return_a_Clone():
    maxElement = 3
    Config.Init(maxElement)
    
    king = King(Constant.WHITE, Constant.KINGWHITECODE)
    result = king.clone()
    assert result.name == king.name
    assert result.value == king.value
    assert result.field == king.field #is None because is not set
    assert result.color.id == king.color.id

    king = King(Constant.WHITE, Constant.KINGWHITECODE, 1)
    result = king.clone()
    assert result.name == king.name
    assert result.value == king.value
    assert result.field.id == king.field.id
    assert result.color.id == king.color.id


def test_setCurrentReachs_Without_Pieces():
    maxElement = 3
    Config.Init(maxElement)
    king = King(Constant.WHITE, Constant.KINGWHITECODE, 5)
    king.setReachs()
    ...
    assert king.currentReachs == [8,9,6,3,2,1,4,7]

def test_setCurrentReachs_With_1_Piece():
    maxElement = 3
    Config.Init(maxElement)
    king = King(Constant.WHITE, Constant.KINGWHITECODE, 5)
    king.setReachs()
    pieces = []
    queenWhite = Queen(Constant.WHITE, Constant.QUEENWHITECODE, 9)
    pieces.append(queenWhite) 
    queenBlack = Queen(Constant.BLACK, Constant.QUEENBLACKCODE, 8)
    pieces.append(queenBlack) 
